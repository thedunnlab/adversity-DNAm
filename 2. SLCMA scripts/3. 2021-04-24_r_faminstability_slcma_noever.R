...
load("~/recoded_faminstability_20210424.Rdata", verbose=T)
head(faminst.recoded.2)

adversity="r_faminstability"
...
faminst.recoded <- faminst.recoded.2[which(faminst.recoded.2$cidB1471 %in% samples.f15.notwins$ALN),]
faminst.recoded <- faminst.recoded[match(samples.f15.notwins$ALN, faminst.recoded$cidB1471),]
adversity.data.2 <- faminst.recoded[,c(1, grep(adversity, colnames(faminst.recoded)))]
